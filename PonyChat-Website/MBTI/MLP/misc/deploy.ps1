# [已废弃] 原 HK-CN2 时代部署脚本；现推荐使用 misc\deploy_mbti_server_usa.py（基于 ssh_lib）。
#
# 构建并上传 web/dist 到服务器（OpenSSH 的 ssh/scp）
# 用法（在 MBTI/MLP 项目根目录 PowerShell）：
#   .\misc\deploy.ps1
#   .\misc\deploy.ps1 -SshTarget "root@154.17.23.237" -RemoteDir "/var/www/mbti"
#
# 注意：本脚本依赖 $bypassPs1 中的 deploy-ssh-bypass.ps1（外部 AIOPC-Website 路径），
#       在当前 PonyChat 仓库中不可用，运行前请先确认路径正确或直接改用 deploy_mbti_server_usa.py。
