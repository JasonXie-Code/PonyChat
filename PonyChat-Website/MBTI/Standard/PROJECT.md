...
- **生产部署**：与美国主站同机 Nginx 时，静态目录多为 **`/var/www/mbti-ponychat-static`**（与 `PonyChat-Website/Main/deploy/server-usa/nginx-ponychat-www.conf` 中 `standard-mbti.ponychat.org` 等站点块一致）；旧配置或 **`misc/deploy.ps1`** 默认可能仍指向旧目录。**`misc/deploy_standard_mbti_server_usa.py`** 负责同步 Standard 变体静态站到 Server-USA。站点片段配置见 **`misc/nginx-mbti.ponychat.org.conf`**（若与主配置合并则以主仓库 `nginx-ponychat-www.conf` 为准）。公网访问域名示例 **`standard-mbti.ponychat.org`**（以实际 DNS 为准）。
