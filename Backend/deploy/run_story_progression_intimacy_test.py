#!/usr/bin/env python3
"""Run the story-progression intimacy matrix test on the remote server and stream output."""
...
try:
    cmd = (
        "cd /opt/ponychat && "
        "PONYCHAT_INTIMACY_TEST_CONCURRENCY=${PONYCHAT_INTIMACY_TEST_CONCURRENCY:-2} "
        "/opt/ponychat/.venv/bin/python -u "
        "/opt/ponychat/Backend/scripts/test_story_progression_intimacy_matrix.py 2>&1"
    )
    args = (
        ["ssh"]
        + _no_proxy_args()
