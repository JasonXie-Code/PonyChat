"""Compatibility loader for `normal_planner scene state persistence` implementation modules.

The implementation lives in `scene_state_persistence_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    ...
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("scene_state_persistence_impl")
