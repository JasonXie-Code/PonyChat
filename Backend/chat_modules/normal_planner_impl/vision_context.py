"""Compatibility loader for `normal_planner vision context` implementation modules.

The implementation lives in `vision_context_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    impl_modules = (
        "vision_context_core.py",
        "planner_reply_policy.py",
    )
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("vision_context_impl")
