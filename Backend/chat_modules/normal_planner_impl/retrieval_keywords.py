"""Compatibility loader for `normal_planner retrieval keywords` implementation modules.

The implementation lives in `retrieval_keywords_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    impl_modules = (
        "retrieval_keyword_core.py",
        "retrieval_keyword_prompt.py",
    )
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("retrieval_keywords_impl")
