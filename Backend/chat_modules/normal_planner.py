"""Compatibility loader for `normal_planner` implementation modules.

The implementation lives in `normal_planner_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    impl_modules = (
        "vision_context.py",
        "scene_intimacy_level.py",
        "fact_judgement.py",
        "scene_state_persistence.py",
        "fact_evidence_markers.py",
        "conversation_plan.py",
        "retrieval_keywords.py",
        "normal_mode_prompt.py",
    )
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("normal_planner_impl")
