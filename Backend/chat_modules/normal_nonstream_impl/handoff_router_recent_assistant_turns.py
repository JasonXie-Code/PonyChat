"""Compatibility loader for `normal_nonstream handoff router` implementation modules.

The implementation lives in `handoff_router_recent_assistant_turns_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    ...
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("handoff_router_recent_assistant_turns_impl")
