"""Compatibility loader for `service normal reply debounce` implementation modules.

The implementation lives in `normal_reply_debounce_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    ...
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("normal_reply_debounce_impl")
