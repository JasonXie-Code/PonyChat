"""Compatibility loader for `admin characters route` implementation modules.

The implementation lives in `characters_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    impl_modules = (
        "admin_character_routes.py",
        "admin_character_updates.py",
    )
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("characters_impl")
