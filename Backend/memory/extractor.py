...
- 旧记忆核对问答本身不创造新事件：用户问“上次/那次在哪里、用了什么、有没有、谁做的、第一次吗”，角色随后给出的回忆答案，无论语气多确定，都不能仅凭这一问一答提取成 episode/activity/relationship。只有用户本轮明确补充或纠正的新事实，或【导演确认的事实边界】明确列出的已核验事实，才可提取；普通的“用户问、角色回答/回忆说 X”也无需另存为长期记忆，避免角色答案反过来成为自己的证据。
    ...
    try:
        ...
        if planner_notes:
            planner_note_block = (
                ...
                "其中标注为已核验事实的内容可用于判断角色的回忆答案是否有独立依据，但它本身不应被重复提取成新事件。"
                "若 assistant 消息与这里冲突，按这里降级为角色临时表达/计划/想法，或省略冲突的地点、姿势、物品状态。\n"
                + planner_notes[:1800]
            )
