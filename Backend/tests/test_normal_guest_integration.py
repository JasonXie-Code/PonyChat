    ...
    async def scenario():
        ...
        async def runner(prompt, model, tools, **kwargs):
            ...
            if private_recall:
                ...
                return model_result('我记得参加了群聊，后来紫悦说了青色晚风。')
            ...
            if later_round and speaker == "twilight":
                ...
                rows = channel.take_pending()
                assert any(row.get("content") == "还有时间问题" for row in rows)
                updated = await channel.prepare_input(rows)
                assert "还有时间问题" in json.dumps(updated, ensure_ascii=False)
                assert channel.try_seal()
            ...
            return model_result("我来评价刚才的话题。" if speaker == "pinkie" else "我接着说这个话题，青色晚风。")
        ...
        if initial is not None:
            ...
            assert live_turn.active_turn("alice", "twilight").routing_waiting
            finish_first.set()
        ...
        async def wait_for_inbox():
            while not live_turn.active_turn("alice", "twilight").pending:
                await asyncio.sleep(0.01)
        await asyncio.wait_for(wait_for_inbox(), 3)
        finish_later.set()
