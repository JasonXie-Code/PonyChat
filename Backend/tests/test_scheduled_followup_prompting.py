"""Compatibility loader for `test_scheduled_followup_prompting` implementation modules.

The implementation lives in `test_scheduled_followup_prompting_impl/` and is executed into this module's
...
def _load_implementation_modules() -> None:
    impl_modules = (
        "prompt_context_tests.py",
        "delivery_prompt_tests.py",
    )
    impl_dir = _impl_pathlib.Path(_impl_sys._getframe().f_code.co_filename).with_name("test_scheduled_followup_prompting_impl")
