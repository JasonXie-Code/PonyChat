...
from prompt_skills_under_test.autonomous_prompt_rules import BODY_EXPRESSION
...
def test_no_character_style_and_body_manual_is_only_loaded_on_request(name):
    ...
    assert BODY_EXPRESSION not in system
    result = asyncio.run(s.load({'name': 'instant_messaging'}))
    assert result['instructions'] == s.catalog['instant_messaging'][1]
    assert s.loaded == {'instant_messaging'}
    assert BODY_EXPRESSION not in result['instructions']
